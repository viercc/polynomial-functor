...
instance (Show v, Show e) => Show (Path v e) where
  showsPrec p (UnsafePath v0 es v1) =
    showParen (p > 10) $ ("Path " ++) . showsPrec 11 v0 . space . showList es . space . showsPrec 11 v1
      where
        space = (" " ++) 
...
mapPath f0 f1 (UnsafePath v0 es v1) = path (f0 v0) (f1 <$> es) (f0 v1)
...
errMapPath f0 f1 p = fromMaybe (error "Invalid morphism between quiver!") $ mapPath f0 f1 p
